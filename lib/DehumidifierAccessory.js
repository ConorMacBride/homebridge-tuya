...
class DehumidifierAccessory extends BaseAccessory {
  ...
  constructor(...props) {
    ...
    this.targetHumidifierDehumidifierStateTimerActive = false;
    this.targetHumidifierDehumidifierStateTimer = null;
    this.relativeHumidityDehumidifierThresholdTimer = null;
    this.rotationSpeedTimerActive = false;
  ...
  _registerCharacteristics(dps) {
    ...
    this.device.on("change", (changes, state) => {
      ...
      if (
        !this.targetHumidifierDehumidifierStateTimerActive &&
        !this.rotationSpeedTimerActive
      ) {
  ...
  setRotationSpeed(value, callback) {
    ...
    this.rotationSpeedTimerActive = true;
    this.rotationSpeedTimer = setTimeout(() => {
      ...
      if (value > 0 && !this.device.state[this.getDp("Active")]) {
        this.characteristicActive.updateValue(true);
        this.setActiveAndWait(5000);
      }

      switch (value) {
        case 0:
          if (origValue > 0)
            // HomeKit only sends active=0 if actually set to 0
            this.setState(this.getDp("Active"), false, () => {});
          break;
        case 50:
          this.setState(this.getDp("FanSpeed"), "low", () => {});
          break;
        case 100:
          this.setState(this.getDp("FanSpeed"), "high", () => {});
          break;
      }
      this.rotationSpeedTimerActive = false;
    }, 1000);
  ...
  setRelativeHumidityDehumidifierThreshold(value, callback) {
    ...
    this.relativeHumidityDehumidifierThresholdTimer = setTimeout(() => {
      ...
      this.setState(this.getDp("Humidity"), adjustedValue, () => {});
    }, 1000);
  ...
  setTargetHumidifierDehumidifierState(value, callback) {
    if (this.targetHumidifierDehumidifierStateTimer) {
      clearTimeout(this.targetHumidifierDehumidifierStateTimer);
      this.targetHumidifierDehumidifierStateTimer = null;
    }
    ...
    this.targetHumidifierDehumidifierStateTimerActive = true;
    this.targetHumidifierDehumidifierStateTimer = setTimeout(() => {
      this.log.info(
        `Executing setTargetHumidifierDehumidifierState for value: ${value}`,
      );

      if (!this.device.state[this.getDp("Active")]) {
        this.characteristicActive.updateValue(true);
        this.setActiveAndWait(10000);
      }

      if (this.characteristicTargetHumidifierDehumidifierState.value !== value)
        this.characteristicTargetHumidifierDehumidifierState.updateValue(value);
