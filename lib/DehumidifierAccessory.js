...
class DehumidifierAccessory extends BaseAccessory {
  ...
  _registerCharacteristics(dps) {
    ...
    this.characteristicRelativeHumidityDehumidifierThreshold = service.getCharacteristic(
    ...
    this.device.on("change", (changes, state) => {
      ...
      if (this.characteristicTargetHumidifierDehumidifierState.value !== mode)
        this.characteristicTargetHumidifierDehumidifierState.updateValue(mode);

      const fanSpeed = this._getRotationSpeed(newState);
      if (this.characteristicRotationSpeed.value !== fanSpeed)
  ...
  setLockPhysicalControls(value, callback) {
    if (!this.device.state[this.getDp("Active")]) {
      this.characteristicLockPhysicalControls.updateValue(false);
      return callback();
    }
    return this.setState(this.getDp("ChildLock"), value ? true : false, callback);
  }
  ...
  setRotationSpeed(value, callback) {
    ...
    switch (value) {
      case 0:
        this.characteristicActive.updateValue(false);
        return this.setState(this.getDp("Active"), false, callback);
      case 50:
        return this.setState(this.getDp("FanSpeed"), "low", callback);
      case 100:
        return this.setState(this.getDp("FanSpeed"), "high", callback);
    }
  }

  setRelativeHumidityDehumidifierThreshold(value, callback) {
    if (this.device.state[this.getDp("ChildLock")] || !this.device.state[this.getDp("Active")]) {
      this.characteristicRelativeHumidityDehumidifierThreshold.updateValue(
    ...
    if (origValue != value) {
      this.characteristicRelativeHumidityDehumidifierThreshold.updateValue(value);
    }
  ...
  setTargetHumidifierDehumidifierState(value, callback) {
    ...
    if (!this.device.state[this.getDp("Active")]) {
      ...
      this.setState(this.getDp("Active"), true, callback); // TODO: is it okay to call the callback twice
    }

    return this.setState(this.getDp("Mode"), this.targetStateToMode(value), callback); // TODO: does this method get called when switched to off?
  }
