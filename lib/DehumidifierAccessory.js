...
class DehumidifierAccessory extends BaseAccessory {
  ...
  _registerCharacteristics(dps) {
    ...
    this.device.on("change", (changes, state) => {
      if (
        this.characteristicChildLock &&
        changes.hasOwnProperty(this.getDp("ChildLock"))
      ) {
        const newChildLock = this._getLockTargetState(
          changes[this.getDp("ChildLock")],
        );
        if (this.characteristicChildLock.value !== newChildLock)
          this.characteristicChildLock.updateValue(newChildLock);
      }
      ...
      if (changes.hasOwnProperty(this.getDp("Active"))) {
        ...
        if (this.characteristicActive.value !== newActive) {
          ...
          const newSpeed = this._getRotationSpeedFromValues(
            newActive,
            newFanSpeed,
          );
          if (this.characteristicRotationSpeed.value !== newSpeed)
            this.characteristicRotationSpeed.updateValue(newSpeed);
        }
      }
      if (
        !changedActiveStatus &&
        changes.hasOwnProperty(this.getDp("FanSpeed"))
      ) {
      ...
      if (
        changes.hasOwnProperty(this.getDp("WaterLevel")) &&
        characteristicWaterLevel.value !==
          this._getTankState(changes[this.getDp("Fault")])
      )
    ...
    setInterval(() => {
      characteristicCurrentHumidity.updateValue(
        this.device.state[this.getDp("CurrentHumidity")],
      );
    }, 30000);
  }
  ...
  _getRotationSpeed(dps) {
    return this._getRotationSpeedFromValues(
      dps[this.getDp("Active")],
      dps[this.getDp("FanSpeed")],
    );
  }
  ...
  setRotationSpeed(value, callback) {
    ...
    if (this.device.state[this.getDp("FanSpeed")] !== fanSpeed)
      return this.setState(this.getDp("FanSpeed"), fanSpeed, callback);
  ...
  getTargetHumidity(callback) {
    this.getState(
      [this.getDp("Active"), this.getDp("Humidity")],
      (err, dps) => {
        if (err) return callback(err);
  ...
  setTargetHumidity(value, callback) {
    if (this.device.state[this.getDp("ChildLock")]) {
      this.characteristicHumidity.updateValue(
        this.device.state[this.getDp("Humidity")],
      );
      return callback();
    }
