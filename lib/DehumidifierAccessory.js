...
class DehumidifierAccessory extends BaseAccessory {
  ...
  constructor(...props) {
    ...
    this._noUpdateRotationSpeed = false;
  }
  ...
  _registerCharacteristics(dps) {
    ...
    this.device.on("change", (changes, state) => {
      ...
      if (!this._noUpdateRotationSpeed && this.characteristicRotationSpeed.value !== fanSpeed)
  ...
  setRotationSpeed(value, callback) {
    ...
    // If not active, wait a bit for HomeKit to set to active
    if (!this.device.state[this.getDp("Active")]) {
      this._noUpdateRotationSpeed = true;
      const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
      (async () => {await sleep(3000)})();
      this._noUpdateRotationSpeed = false;
      if (!this.device.state[this.getDp("Active")]) return callback(true);
    }
  ...
  setRelativeHumidityDehumidifierThreshold(value, callback) {
    this.log.info(`setRelativeHumidityDehumidifierThreshold: ${value}`)
    if (
      ...
      if (
        !(async () => {
          ...
          while (attempts < 4) {
            ...
            if (this.device.state[this.getDp("Active")]) return true;
            attempts++;
          }
          this.log.info(
            "Took too long to set Active to true before setting Mode, not setting mode.",
          );
          return false;
        })()
      )
